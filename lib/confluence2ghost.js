...
// FIXME: workaround to https://github.com/domchristie/to-markdown/issues/46
function convertNodesSeparately(xhtml) {
  var dom = asDom(xhtml);
  var markdown = '';
  var xmlSerializer = new XMLSerializer();
  var nodes = dom.getElementsByTagName('div')[0].childNodes;
  for (var i = 0; i < nodes.length; i++) {
    var elementXHTML = xmlSerializer.serializeToString(nodes[i]);
    markdown += toMarkdown(elementXHTML) + '\n';
  }
  return markdown;
}
...
exports.convert = function (confluenceXHTML) {
  ...
  fs.writeFile('xhtml.xml', xhtml, function (err) {
    if (err) {
      console.log(err);
    } else {
  ...
  return convertNodesSeparately(xhtml);
};
